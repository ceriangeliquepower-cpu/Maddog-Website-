...
2. **Core Web Vitals — mobile LCP, in progress, real progress made.** Font-loading fix (render-blocking Google Fonts stylesheet) pushed & live 2026-09-17: mobile LCP went from 10.2s → 5.0s (confirmed via live PageSpeed test, both runs on the homepage). Also pushed same day: image caching fix (Hard-Won Rule #13), hero-image preload + `fetchpriority="high"` on the 4 highest-traffic pages (home, Amanda, training hub, recovery). **5.0s is still "Poor" by Google's threshold (<2.5s is "Good")** — real gap remains. Next levers, not yet done:
   - Image delivery — PageSpeed's own "Improve image delivery" insight estimated ~587 KiB of savings on mobile (likely resizing/format, not just compression quality) — needs its own investigation, bigger job, do deliberately not as a quick add-on.
   - Hero preload only applied to 4 pages — other pages with a large above-fold hero image (training discipline pages, wellness pages, coaches.html) haven't been checked/fixed the same way.
   - Two Lighthouse findings never pinpointed to an exact element: a color-contrast issue and a heading-order (non-sequential) issue. Confirmed via the same audit: `<main>` landmark and 2 non-descriptive links were pinpointed and fixed 2026-09-17 (see below); contrast and heading-order were not — need the full Lighthouse report (not just the summary) to find the exact elements.
   - `<main>` landmark fixed 2026-09-17 but **gym-site pages only (32 of them)** — the 19-20 wellness pages use a different template (`w-footer`, no `site-nav-mobile`) and were deliberately left for a separate, separately-verified pass rather than guessing at their structure.
