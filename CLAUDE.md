...
8. **Personal training added as a tracked ranking priority (client request, 2026-09-17)** — currently weak: Maddog sits ~#6-7 organically on "personal trainer ballito" / "personal training ballito" / "personal training classes ballito" / "personal training studio ballito", behind **Grit Factory** (#1 on all of them — one strong benefit-led page + reviews, not content depth, so beatable). Full competitor breakdown in [[project_personal_training_competitor_audit]] in memory. `training-personal-training-ballito.html` strengthened 2026-09-17: sharpened title/meta/H1 to cover the weak exact-match phrases, and — since client confirmed **Amanda "Maddog" Lino is also a personal trainer at Maddog** (already documented in her own JSON-LD on `amanda.html`, just never surfaced on this page) — added a full second-coach section for her plus a matching FAQ/schema entry, which directly resolves the "female personal trainer ballito" gap this audit flagged. Local edit only, not yet pushed. Still open: no neurodiversity/special-needs coaching angle (Launch Lifestyle owns this — "Neurodiverse Specialist Coaching," ADHD/autism/sensory — only add if Maddog genuinely offers/wants to offer this, don't claim it speculatively); no visible testimonials section despite schema claiming 5.0★/8 reviews (need real client quotes from the client to add honestly). Body20's EMS/20-minute-session angle is a different model, not directly competable but worth knowing about.
